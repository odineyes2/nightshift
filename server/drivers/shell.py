...
셸 파드는 정의상 **임의 명령 실행**이다. 그래서 환경변수로 명시적으로 켜야만 등록할 수 있게
했고, 켜도 관리자(admin)만 만들고 쓸 수 있다(app.py가 일반 회원의 셸 파드 생성을 막는다):

    NIGHTSHIFT_ENABLE_SHELL_PODS=1

관리자 계정의 비밀번호(NIGHTSHIFT_ADMIN_PASSWORD)는 충분히 길게 잡을 것.
