...
- **대기 큐**: 파드를 안 정한 작업은 **일시정지(`pending`) 상태로 만들어집니다** — 빈 파드가 있어도 바로 돌지 않습니다. 작업의 ▶(`POST /api/jobs/{id}/start`)나 큐 ▶ 시작을 누르면 `queued`+`pod_id: null`이 되고("파드 대기" 배지), ⏸ 일시정지(`POST /api/jobs/{id}/pause`)로 다시 `pending`으로 돌릴 수 있습니다(스케줄러가 골라 준 파드는 풀림, 사람이 고정한 파드는 유지). 서버의 스케줄러(`schedule_once`, `NIGHTSHIFT_SCHED_INTERVAL_SEC`초마다, 기본 5)가 등록 순서대로 훑으며 아래를 **모두** 만족하는 파드가 나오면 그 파드로 배정하고 그 파드 큐에 넣습니다.
