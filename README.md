...
- `.env`에 `NIGHTSHIFT_API_KEY`를 설정하면 모든 `/api/*` 요청에 같은 값을 `X-API-Key` 헤더(또는 `Authorization: Bearer` 헤더)로 요구합니다. 비워두면(기본값) 기존처럼 인증 없이 실행되며, 서버 시작 로그에 경고가 남습니다. 정적 파일(`/`, 브라우저 UI 자체)은 이 검사 대상이 아니지만, UI의 `fetch` 호출들은 아직 이 헤더를 붙이지 않으므로 키를 설정하면 브라우저 화면의 API 호출이 401로 막힙니다 — 브라우저로 계속 조작하려면 프런트엔드에도 헤더 전송 처리가 추가로 필요합니다.
