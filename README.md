...
13. 화면 맨 위의 **"삭제된 작업 설정 불러오기"** 패널(기본 접힘)에서는, "작업 목록"에서 삭제한 작업의 설정도 드롭다운으로 골라 똑같이 불러올 수 있습니다. "삭제"는 실제로는 소프트 삭제라서(위 10번 참고) 워크플로우/CSV 파일이 곧바로 지워지지 않고, 가장 최근에 삭제한 순서로 최대 `NIGHTSHIFT_DELETED_JOBS_RETENTION`개(기본 30, 패널에 현재 값이 표시됨)까지 서버에 남아있습니다 — 그보다 오래전에 삭제된 것은 디스크가 무한정 늘어나지 않도록 자동으로 완전히 정리됩니다. 드롭다운에서 하나를 고르고 "이 설정 불러오기"를 누르면 12번과 똑같이 동작합니다(등록 아님, 예외 처리 동일).
