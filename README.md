...
- **생성 카드 — 보드에서 바로 참조 생성하기**: 보드의 이미지를 참조로 새 이미지·영상을 만들 때 작업 화면으로 가지 않아도 되게 합니다. 템플릿마다 화면을 따로 만들지 않고, 사용자가 쓰던 템플릿·워크플로우를 **보드 프리셋**으로 저장해 꺼내 쓰는 방식입니다.
  - **프리셋 만들기**: 새 작업 창에서 평소처럼 템플릿·워크플로우·옵션을 정한 뒤 "2. 세부 설정"의 **보드 프리셋으로 저장**(폰에서는 "프리셋")을 누릅니다. 이름과 **카드에서 바로 고칠 옵션**(글·숫자·선택 옵션)을 고르면, 나머지 값은 지금 폼의 값으로 고정되고 워크플로우는 사본으로 저장됩니다. 템플릿의 입력 이미지 칸은 보드에서 선으로 채우는 **입구**가 됩니다. 프리셋은 회원별이라 어느 프로젝트 보드에서나 쓰고, 같은 이름으로 다시 저장하면 덮어쓸지 묻고, 저장 창 아래 목록에서 지웁니다. CSV로 여러 줄을 도는 템플릿은 프리셋으로 쓸 수 없습니다.
...
- **카드 옮기기·지우기**: 카드를 끌어 옮깁니다. 텍스트 카드도 어디를 잡든 끌리고, 본문을 끌지 않고 톡 누르면 글쓰기가 시작됩니다(위쪽 점무늬 띠를 톡 누르면 글쓰기 없이 고르기만 하고, 글쓰기 중에 옮기려면 그 띠를 잡습니다. 글쓰기 중 `Esc`는 글쓰기만 끝냅니다). 카드에 마우스를 올리면 오른쪽 위에 휴지통이 나옵니다.
