...
> ⚠️ **셸 파드는 기본으로 꺼져 있습니다.** 정의상 임의 명령 실행이라, nightshift가 인증 없이
> 떠 있으면(=`NIGHTSHIFT_API_KEY`가 비어 있으면) "그 포트에 닿는 누구나 이 머신에서 아무 명령이나
> 돌릴 수 있는" 상태가 됩니다. 쓰려면 `NIGHTSHIFT_ENABLE_SHELL_PODS=1`로 명시적으로 켜야 하고,
> **켤 거라면 `NIGHTSHIFT_API_KEY`도 반드시 함께 설정하세요.**
...
- `.env`에 `NIGHTSHIFT_API_KEY`를 설정하면 모든 `/api/*` 요청에 같은 값을 `X-API-Key` 헤더(또는 `Authorization: Bearer` 헤더, 또는 `?api_key=` 쿼리스트링)로 요구합니다. 비워두면(기본값) 기존처럼 인증 없이 실행되며, 서버 시작 로그에 경고가 남습니다. 정적 파일(`/`, 브라우저 UI 자체)은 이 검사 대상이 아닙니다. 브라우저 UI는 헤더 우측의 "🔑 API 키" 버튼으로 키를 입력하면 이 브라우저의 `localStorage`에 저장해두고 이후 모든 API 호출에 자동으로 붙입니다(서버에는 저장되지 않음) — 키가 없거나 틀리면 이 버튼이 빨간색으로 바뀝니다.
  - **쿼리스트링도 받는 이유**: 갤러리 썸네일/원본 이미지/영상(`<img src>`/`<video src>`)과 다운로드 링크(`<a href download>`)는 브라우저가 커스텀 헤더 없이 직접 요청을 보내므로, 자바스크립트가 `X-API-Key` 헤더를 붙여줄 방법이 없습니다. 그래서 이 URL들만 `index.html`이 `?api_key=...`를 직접 붙여서 만듭니다(`window.__nightshiftMediaUrl` 헬퍼). API 키를 켠 뒤 갤러리 썸네일이 전부 깨진 아이콘으로 보인다면, 이 부분이 없던 옛 버전일 가능성이 큽니다 — 최신 코드로 업데이트하세요.
