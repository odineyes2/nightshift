...
- 대기/실행 중인 작업(pending+queued+running) 합계가 1000개(`app.py`의 `MAX_ACTIVE_JOBS` 상수) 이상이면 `POST /api/upload`가 429로 새 작업 추가를 거부합니다. `worker_loop`가 한 번에 하나씩만 순차 실행하는 구조라, 반복 호출하는 스크립트나 LLM의 버그로 큐가 한없이 쌓여 GPU 시간을 낭비하는 걸 막기 위한 안전장치입니다. 숫자를 바꾸고 싶으면 `app.py`에서 그 상수만 고치면 됩니다.
