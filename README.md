...
- 칸을 클릭(정확히는 썸네일 이미지를 클릭)하면 원본 크기로 볼 수 있는 라이트박스가 열립니다. 파일명·용량·수정 시각·전체 중 몇 번째인지가 하단에 표시되고, 좌우 화살표 버튼이나 키보드 방향키(←/→)로 이전/다음 이미지로 이동, `Esc`나 바깥 영역 클릭으로 닫을 수 있습니다. `Delete`(맥 키보드처럼 `Backspace`만 있는 경우도 인식)를 누르면 지금 보고 있는 이미지를 바로 삭제합니다 — 버튼으로 누르는 것과 동일하게 확인창을 거치고, 삭제되면 라이트박스가 닫힙니다.
- **선택 / 다운로드 / 삭제**: 각 칸 왼쪽 위 체크박스로 여러 장을 고를 수 있고(체크박스나 오른쪽 위 삭제(×) 아이콘을 눌러도 라이트박스는 열리지 않습니다), 하나라도 선택하면 격자 위에 툴바가 나타나 **"다운로드"**(1장이면 원본 파일 그대로, 여러 장이면 zip으로 묶어서), **"삭제"**(선택한 것 모두, 확인창 거침), **"선택 해제"**를 제공합니다. 칸마다 있는 삭제(×) 아이콘으로는 선택 없이 그 이미지 한 장만 바로 지울 수 있고, 라이트박스에도 지금 보고 있는 이미지의 **"다운로드"**/**"삭제"** 버튼이 있습니다(삭제하면 라이트박스가 닫히고 격자가 갱신됨). 다운로드는 항상 원본 파일을 받습니다(격자에 보이는 축소본이 아님). 삭제는 전부 되돌릴 수 없어 브라우저 확인창을 거칩니다.
