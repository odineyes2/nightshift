...
- 탭을 열 때마다 `GET /api/output-images`로 목록을 새로 받아오고("새로고침" 버튼으로 수동 새로고침도 가능), 최신 이미지가 먼저 오도록 수정 시각 기준 내림차순으로 정렬합니다. 이미지가 하나도 없으면 안내 문구만 보여줍니다.
...
| `GET` | `/api/output-images/{filename}/thumbnail?size=320` | 그 이미지를 요청마다 즉석에서 축소해 JPEG로 반환(갤러리 격자용, 디스크에 캐시하지 않음). `size`는 긴 변 기준 픽셀(64~800 사이로 clamp, 기본 320) |
