...
- 대기 중인 작업의 워크플로우 JSON을 웹 UI에서 바로 열어 편집/저장 (실행 중/완료/실패 작업은 수정 불가)
...
10. **상태가 "대기중"인 작업**에는 "워크플로우 수정" 버튼이 함께 나타납니다. 누르면 모달이 열리고 워크플로우 JSON 전체가
    편집 가능한 텍스트 영역에 표시됩니다. 문법이 깨지면 텍스트 영역 테두리가 빨갛게 바뀌고 어느 줄/열 근처가
    문제인지 표시되며, 고쳐지기 전까지 "저장" 버튼이 비활성화됩니다. 저장하면 모달이 닫히고 목록이 새로고침됩니다.
    실행 중이거나 이미 끝난 작업은 이 버튼이 보이지 않습니다(서버도 대기 중이 아닌 작업의 수정 요청은 거부합니다).
...
| `PUT` | `/api/jobs/{job_id}/workflow` | 워크플로우 JSON을 덮어씀 (요청 본문 = 새 JSON 텍스트). 작업 상태가 `queued`가 아니면 400, 유효한 JSON이 아니면 400 |
