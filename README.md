...
- 다음은 의도적으로 뺐습니다 — 폭/높이 표시와 "📋 자세히" 보기(둘 다 `ffprobe` 없이는 읽을 수 없는데, 이 기능을 위해 새 시스템 의존성을 추가하지 않기로 했습니다), 회전 후 다운로드(이미지 전용 후처리), "📌 세트 추가"(포즈 참조용, 이미지 전용), "📂 작업 불러오기"(영상 작업 템플릿이 아직 없어 불러올 설정이 없습니다).
