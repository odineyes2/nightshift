...
| `POST` | `/api/projects/{id}/board/nodes` | 카드를 놓는다(`{"kind": "image"\|"video"\|"text"\|"job"\|"frame", "asset_path", "job_id", "text", "x", "y", "width", "height"}`, frame은 묶음 틀이고 제목은 text). 이미지/영상은 자기(admin은 전부) 결과물만, 작업 카드는 자기 작업만 받는다 |
| `GET` | `/api/projects/{id}/board/jobs` | 보드의 작업 카드마다 지금 상태 `{"cards": {카드 id: {job_id, status, active, template_label, prompt, progress, queued_at, started_at, finished_at, pod_name, results: [{path, kind, nsfw}]}}}` — 작업이 지워졌거나 볼 수 없으면 `missing: true` |
...
| `PATCH`/`DELETE` | `/api/projects/{id}/board/nodes/{node_id}` | 카드 위치·크기·글(`x`/`y`/`width`/`height`/`text`) 수정 / 삭제(붙은 선도 같이 삭제) |
| `POST` | `/api/projects/{id}/board/edges` | 두 카드를 잇는다(`{"from_node_id", "to_node_id"}`). 둘 다 이 프로젝트 카드여야 하고, 같은 두 카드 사이에 이미 선이 있으면 그 선을 돌려준다 |
