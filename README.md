...
- 아직 없는 것: 묶음(그룹), 작업(Job) 카드, 여러 사람 동시 편집.
...
| `POST` | `/api/projects/{id}/board/nodes` | 카드를 놓는다(`{"kind": "image"\|"video"\|"text", "asset_path", "text", "x", "y", "width", "height"}`). 이미지/영상은 자기(admin은 전부) 결과물만 받는다 |
