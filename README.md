...
12. **모든 작업**(상태와 무관하게)에는 "설정 불러오기" 버튼이 있습니다. 누르면 "새 작업 추가" 폼이 그 작업의 템플릿 + 옵션 + 워크플로우 + CSV로 채워집니다 — 직전에 돌린 것과 거의 같은 설정으로, 값 한두 개만 바꿔서 다시 등록하고 싶을 때 매번 파일을 다시 첨부하고 옵션을 처음부터 입력할 필요가 없게 해줍니다.
    - **불러오기는 등록이 아닙니다.** 폼을 채울 뿐 아무 작업도 큐에 넣지 않으며, 여전히 "큐에 추가" → "▷ 대기중 작업 시작" 두 단계를 그대로 거쳐야 실행됩니다. 등록 시점에는 새 `job_id`가 생기고 워크플로우/CSV도 서버에 별도 사본으로 저장되므로("워크플로우 수정"으로 고쳐도), 불러온 원본 작업에는 전혀 영향이 없습니다.
