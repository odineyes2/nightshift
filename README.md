...
- **되돌리기 / 다시 하기**: 툴바 버튼이나 `Ctrl`/`⌘`+`Z`(되돌리기), `Ctrl`/`⌘`+`Shift`+`Z` 또는 `Ctrl`+`Y`(다시 하기). 카드 옮기기(여러 장이면 한 번에)·놓기·지우기, 글쓰기 한 번(글칸에 들어갔다 나올 때까지), 선 잇기·끊기를 되돌립니다. 지운 카드를 되돌리면 붙어 있던 선도 같이 살아납니다. 글쓰기 중의 `Ctrl`+`Z`는 글칸 안의 글자 되돌리기입니다. 기록은 최근 100개까지, 지금 열어 둔 보드 안에서만 유지되고 새로고침하거나 다른 프로젝트로 가면 비워집니다. 되돌리기를 서버에 저장하지 못하면 알림을 띄우고 저장된 상태로 다시 불러오며 기록을 비웁니다.
...
| `POST` | `/api/projects/{id}/board/nodes` | 카드를 놓는다(`{"kind": "image"\|"video"\|"text"\|"job", "asset_path", "job_id", "text", "x", "y", "width", "height"}`). 이미지/영상은 자기(admin은 전부) 결과물만, 작업 카드는 자기 작업만 받는다 |
