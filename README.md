...
- 칸을 클릭하면 원본 크기로 볼 수 있는 라이트박스가 열립니다. 파일명·용량·수정 시각·전체 중 몇 번째인지가 하단에 표시되고, 좌우 화살표 버튼이나 키보드 방향키(←/→)로 이전/다음 이미지로 이동, `Esc`나 바깥 영역 클릭으로 닫을 수 있습니다.
- 보기 전용입니다 — 이미지 삭제/회전/다운로드는 기존처럼 "작업 관리" 탭의 "결과 이미지 관리"/"결과 이미지 ZIP 다운로드" 패널에서 합니다.
