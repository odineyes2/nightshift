...
1. 브라우저에서 서버 주소(`http://<host>:8000`)에 접속합니다. 상단에 **"작업 관리"**/**"갤러리"** 탭이 있습니다 — 아래 2~13번은 "작업 관리" 탭 기준이고, 갤러리는 맨 아래 별도 절에서 설명합니다.
...
10. 아직 시작하지 않았거나(대기중) 완료/실패/중단된 작업은 "삭제" 버튼으로 "작업 목록"에서 제거할 수 있습니다. 이미 시작됐거나 실행 중인 작업은 삭제할 수 없습니다. (배치를 시작하기 전이라면 대기 목록에서 특정 작업만 빼고 싶을 때 이 버튼으로 제외할 수 있습니다.) 실제로는 목록에서만 사라지는 소프트 삭제입니다 — 아래 "삭제된 작업 설정 불러오기" 참고.
...
12. **모든 작업**(상태와 무관하게)에는 "설정 불러오기" 버튼이 있습니다. 누르면 "새 작업 추가" 폼이 그 작업의 템플릿 + 옵션 + 워크플로우 + CSV로 채워집니다 — 직전에 돌린 것과 거의 같은 설정으로, 값 한두 개만 바꿔서 다시 등록하고 싶을 때 매번 파일을 다시 첨부하고 옵션을 처음부터 입력할 필요가 없게 해줍니다.
...
13. "작업 관리" 탭 맨 위의 **"삭제된 작업 설정 불러오기"** 패널(기본 접힘)에서는, "작업 목록"에서 삭제한 작업의 설정도 드롭다운으로 골라 똑같이 불러올 수 있습니다. "삭제"는 실제로는 소프트 삭제라서(위 10번 참고) 워크플로우/CSV 파일이 곧바로 지워지지 않고, 가장 최근에 삭제한 순서로 최대 `NIGHTSHIFT_DELETED_JOBS_RETENTION`개(기본 30, 패널에 현재 값이 표시됨)까지 서버에 남아있습니다 — 그보다 오래전에 삭제된 것은 디스크가 무한정 늘어나지 않도록 자동으로 완전히 정리됩니다. 드롭다운에서 하나를 고르고 "이 설정 불러오기"를 누르면 12번과 똑같이 동작합니다(등록 아님, 예외 처리 동일).
