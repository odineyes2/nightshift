...
- 대기/실행 중인 작업(pending+queued+running) 합계가 `NIGHTSHIFT_MAX_ACTIVE_JOBS`(기본 1000) 이상이면 `POST /api/upload`가 429로 새 작업 추가를 거부합니다. `worker_loop`가 한 번에 하나씩만 순차 실행하는 구조라, 반복 호출하는 스크립트나 LLM의 버그로 큐가 한없이 쌓여 GPU 시간을 낭비하는 걸 막기 위한 안전장치입니다.
