...
시드)에 input_image_batch.py의 입력 이미지 주입을 결합한다. CSV 행마다 input_image
컬럼으로 img2img/USDU에 쓸 입력 이미지를 지정한다(세트 구분 없는 평평한 목록 —
...
CSV 컬럼:
    ...
    input_image      (필수, 모든 행) 이 행에 쓸 입력 이미지 파일명 —
                     ...
                     것을 허용하지 않는다 — img2img/USDU는 입력 이미지 없이는
                     애초에 성립하지 않는 워크플로우이기 때문이다.

해상도(WIDTH/HEIGHT)를 다루지 않는 이유:
    workflow_builder.py가 img2img/usdu 모드로 만드는 워크플로우에는 EmptyLatentImage
