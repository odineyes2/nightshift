...
영상 해상도는 시작 프레임 이미지의 실제 크기에서 계산한다(wan22_video_csv_batch.py가
start_image 기준으로 계산하는 것과 동일한 정책 — 끝 프레임 크기는 보지 않음).
...
CSV 컬럼:
    ...
    trigger_prompt, negative_prompt
                    ...
                    건너뛴다. 행 처리나 큐 자체는 멈추지 않음)
    quality_prompt  이미지 생성 부가 프롬프트, 시작/끝 공통 (선택 — csv_batch.py와의
                    컬럼 호환을 위해 남겨둔 부가 컬럼일 뿐, 필수 아님)
    seed            시작 프레임 이미지 생성 시드 (선택, 비어 있으면 무작위)
    end_seed        끝 프레임 이미지 생성 시드 (선택, 비어 있으면 무작위)
    width, height   이미지 생성 해상도, 시작/끝 공통 (선택, 둘 다 채워야 적용)
    resolution      이미지 생성 해상도 프리셋/WxH, 시작/끝 공통 (선택)
    user_prompt(=video_prompt)
                    영상 생성 프롬프트 (선택 — 비어 있으면 main_prompt/prompt(시작
                    프레임 프롬프트)를 그대로 재사용한다. video_prompt는 예전
                    컬럼명과의 호환을 위해 계속 받아준다)
    video_seed      영상 생성 시드 (선택, 비어 있으면 무작위)

    위 컬럼들 외에 CSV에 다른 컬럼이 더 있어도 그냥 무시된다 — 그것 때문에
    행이 건너뛰어지거나 큐 처리가 멈추는 일은 없다.

    영상 해상도는 항상 시작 프레임 이미지의 실제 크기에서 자동 계산되며,
    별도 컬럼으로 지정할 수 없다.
...
VIDEO_WORKFLOW_PATH = Path(__file__).resolve().parent / "video_workflows" / "wan22_flf2v.json"
...
def apply_prompts(workflow, row):
    ...
    for field, title_substring in PROMPT_FIELD_TITLES.items():
        ...
        if input_field is None:
            print(f"[img2video_flf2v_csv] 경고: '{field}' 값을 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
            continue
        node.setdefault("inputs", {})[input_field] = value
        applied = True
    if not applied:
        print("[img2video_flf2v_csv] 경고: 이 행에 이미지 생성 프롬프트 컬럼 값이 하나도 없습니다.", file=sys.stderr)
...
def apply_seed(workflow, seed):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 이미지 생성 시드를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
    try:
        node.setdefault("inputs", {})["seed"] = int(seed)
    except (TypeError, ValueError):
        print(f"[img2video_flf2v_csv] 경고: seed 값 '{seed}'을 정수로 변환하지 못했습니다.", file=sys.stderr)
...
def resolve_resolution(width, height, resolution):
    ...
    if width and height:
        try:
            ...
            print(
                f"[img2video_flf2v_csv] 경고: width/height 값 '{width}x{height}'을 정수로 변환하지 못했습니다. "
                "resolution 컬럼으로 대체합니다.", file=sys.stderr,
            )
    elif width or height:
        print(
            "[img2video_flf2v_csv] 경고: width/height는 둘 다 채워야 적용됩니다. resolution 컬럼으로 대체합니다.",
            file=sys.stderr,
        )
    if not resolution:
        return None
    ...
    if not match:
        print(f"[img2video_flf2v_csv] 경고: resolution 값 '{resolution}'을 해석하지 못했습니다.", file=sys.stderr)
        return None
...
def apply_resolution(workflow, width, height, resolution):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 이미지 해상도를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
...
def apply_checkpoint(workflow):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 체크포인트를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
...
def apply_lora(workflow):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 이미지 생성 LoRA를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
    if lora:
    ...
    if strength:
        try:
            ...
            print(f"[img2video_flf2v_csv] 경고: LORA_STRENGTH 값 '{strength}'을 숫자로 변환하지 못했습니다.", file=sys.stderr)
...
# ── 영상 생성 단계 (wan22_video_csv_batch.py와 동일, fast_4step만 없음) ──

def resolve_video_prompt(row):
    """영상 생성에 쓸 프롬프트를 결정한다: user_prompt(=video_prompt)가 있으면
    그 값을, 비어 있으면 시작 프레임에 쓴 main_prompt/prompt를 그대로
    재사용한다."""
...
def apply_video_user_prompt(workflow, prompt):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: user_prompt 노드를 찾지 못했습니다.", file=sys.stderr)
        return
...
def apply_video_seed(workflow, seed):
    node_id, node = find_node(workflow, title_substring="KSampler", class_types=("KSampler", "KSamplerAdvanced"))
    if node is None:
        print("[img2video_flf2v_csv] 경고: 영상 시드를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
    ...
    for title in ("user_lora_high", "user_lora_low"):
        ...
        if lora_strength:
            try:
                ...
                print(f"[img2video_flf2v_csv] 경고: VIDEO_LORA_STRENGTH 값 '{lora_strength}'을 숫자로 변환하지 못했습니다.", file=sys.stderr)
...
def apply_text_enhance(workflow, prompt, enabled):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 긍정 프롬프트 노드를 찾지 못해 텍스트 인핸스를 끄지 못했습니다.", file=sys.stderr)
        return
...
def apply_video_size(workflow, width, height):
    ...
    if node is None:
        print("[img2video_flf2v_csv] 경고: 영상 해상도를 넣을 노드를 찾지 못했습니다.", file=sys.stderr)
        return
    inputs = node.setdefault("inputs", {})
    inputs["width"] = width
...
def wait_for_completion(comfy_url, prompt_id):
    ...
    while time.time() < deadline:
        req = urllib.request.Request(f"{comfy_url}/history/{prompt_id}", headers={"User-Agent": COMFY_USER_AGENT})
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
...
def report_progress(job_id, nightshift_url, total, done):
    ...
    try:
        ...
        print(f"[img2video_flf2v_csv] 경고: 진행 상황 보고 실패: {e}", file=sys.stderr)
...
def generate_image(image_workflow, comfy_url, row, title, index, suffix, seed):
    ...
    print(f"[img2video_flf2v_csv] [{index}] {suffix} 이미지 생성 큐 등록 (prompt_id={prompt_id})")
    ...
    print(f"[img2video_flf2v_csv] [{index}] {suffix} 이미지 생성 완료 → {uploaded_name}")
...
def run_once(image_workflow, video_workflow, comfy_url, row, title, index, start_seed, end_seed, video_seed,
    ...
    for img_title, uploaded_name in (("start_image", start_name), ("end_image", end_name)):
        node_id, node = find_node(video, title_substring=img_title, class_types=("LoadImage",))
        if node is None:
            raise RuntimeError(f"영상 워크플로우에 {img_title} 노드가 없습니다.")
        node.setdefault("inputs", {})["image"] = uploaded_name

    video_dims = compute_video_dims_from_bytes(start_bytes)
    ...
    print(f"[img2video_flf2v_csv] [{index}] 영상 생성 큐 등록 (prompt_id={prompt_id})")
    wait_for_completion(comfy_url, prompt_id)
    print(f"[img2video_flf2v_csv] [{index}] 영상 생성 완료")
...
def main():
    ...
    for line_no, row in enumerate(rows, start=2):
        ...
        start_seed_raw = (row.get("seed") or "").strip()
        start_seed = int(start_seed_raw) if start_seed_raw else random.randint(0, 2**31 - 1)
        end_seed_raw = (row.get("end_seed") or "").strip()
        end_seed = int(end_seed_raw) if end_seed_raw else random.randint(0, 2**31 - 1)
        video_seed_raw = (row.get("video_seed") or "").strip()
        video_seed = int(video_seed_raw) if video_seed_raw else random.randint(0, 2**31 - 1)
        plan.append((row, title, start_seed, end_seed, video_seed))

    print(f"[img2video_flf2v_csv] 총 {len(plan)}건 제출 예정")
    report_progress(job_id, nightshift_url, len(plan), 0)

    done = 0
    for index, (row, title, start_seed, end_seed, video_seed) in enumerate(plan, start=1):
        run_once(image_workflow, video_workflow, comfy_url, row, title, index, start_seed, end_seed, video_seed,
                 video_lora_name, video_lora_strength, text_enhance)
        done += 1
        report_progress(job_id, nightshift_url, len(plan), done)

    print(f"[img2video_flf2v_csv] 총 {len(plan)}건 완료")
