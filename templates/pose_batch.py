...
def apply_checkpoint(workflow):
    ...
    if node is None:
        print("[pose_batch] 경고: 체크포인트를 넣을 노드를 찾지 못했습니다 (CheckpointLoader 없음)", file=sys.stderr)
        return
...
def apply_lora(workflow):
    ...
    if node is None:
        print("[pose_batch] 경고: LoRA를 넣을 노드를 찾지 못했습니다 (LoraLoader 없음)", file=sys.stderr)
        return
    total = sum(
    ...
    if total > 1:
        print(
            f"[pose_batch] 안내: LoRA 로더가 {total}개라 그중 노드 {node_id}에만 적용했습니다 "
    ...
    if strength:
        try:
            ...
            print(f"[pose_batch] 경고: LORA_STRENGTH 값 '{strength}'을 숫자로 변환하지 못했습니다.", file=sys.stderr)
        else:
