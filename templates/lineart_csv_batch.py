"""
CSV + lineart 배치 템플릿 — csv_batch.py의 CSV 기반 배치 제출(프롬프트 여러 개, 시드,
해상도, 배치수)에 lineart_batch.py의 lineart 레퍼런스 주입을 결합한다. CSV 행마다
lineart 컬럼으로 ControlNet에 쓸 lineart 레퍼런스 이미지를 지정할 수 있고, 비어
있으면 그 행은 ControlNet 없이(비활성화) 생성한다. pose_csv_batch.py/depth_csv_batch.py와
완전히 동일한 구조를 갖는 독립 스크립트다 — "주(main) 참조"만 lineart로 고정돼 있다.
...
CSV 컬럼 (csv_batch.py와 동일 + lineart):
...
lineart 레퍼런스 폴더 구조(char_no로 스코프됨):
    ...
    (레거시/개별 override: NIGHTSHIFT_LINEART_DIR을 설정하면 대신 그 값을 씀)
        1/                  1인물(solo) 세트들
            <세트>/<파일>...
...
참조 해석(lineart/secondary_ref 컬럼 값, resolve_ref_reference(kind_dir, char_no, name)):
    ...
    lineart/char_no 값 해석은 잡을 큐에 올리는 시점(app.py 업로드 검증)에 CSV
    전체를 미리 한 번 돌려 확인하지만, 그 사이 lineart 폴더 내용이 바뀌었을 수
...
환경변수:
    ...
    LINEART_MODE       "sequential" 또는 "random" (nightshift가 템플릿 옵션 "lineart_mode"로
                       주입, 기본 sequential) — lineart 컬럼에 세트 이름만 지정한 행에 적용
    ...
    NIGHTSHIFT_OUTPUT_DIR  ComfyUI가 이미지를 저장하는 폴더 (기본 /workspace/output).
                       재현성 기록용 jsonl(lineart_csv_batch_manifest.jsonl)을 여기 같이 남긴다
    ...
    LINEART_NODE_TITLE lineart 이미지를 주입할 LoadImage 노드의 _meta.title 부분일치
                       ...
                       하나뿐일 때 그 노드를 대신 쓴다)
    CONTROLNET_NODE_TITLE  lineart가 비어 있을 때 비활성화(strength=0)할 노드의
...
ComfyUI로의 lineart 이미지 주입 방식:
    lineart_batch.py와 동일 — LoadImage가 참조하는 파일은 ComfyUI 자신의 input
    폴더에 있어야 하므로, 파일시스템 공유 여부와 무관하게 항상 동작하도록
...
ControlNet 비활성화(lineart가 비어 있는 행):
...
재현성 기록:
    lineart_batch.py와 같은 형식으로 NIGHTSHIFT_OUTPUT_DIR에
    lineart_csv_batch_manifest.jsonl을 이어쓰기(append)한다. lineart가 비어 있던 행은
    char_no/lineart_set/lineart_file을 모두 null로 남겨 "의도적으로 ControlNet 없이
...
보조 참조(SECONDARY_KIND 등) — lineart와 pose/depth를 한 생성에 같이 쓰기:
    이 템플릿의 "주(main) 참조"는 항상 lineart다(위 lineart/char_no 컬럼). 그와 별개로,
    SECONDARY_KIND를 "none"이 아닌 값으로 설정하면 CSV의 secondary_ref/
    secondary_char_no 컬럼이 활성화되어, 행마다 pose나 depth 레퍼런스를
    "보조 참조"로 하나 더 얹을 수 있다. 워크플로우에서 제목에 SECONDARY_NODE_TITLE
    (기본 "secondary")이 포함된 LoadImage 노드를 찾아 주입하며, 이 노드는 lineart_batch.py와
    ...
    secondary_ref 값 자체는(노드 존재 여부와 별개로) lineart 컬럼과 같은 엄격함으로
...
# 제네릭한 구현이라 주 참조(lineart)/보조 참조 해석에 똑같이 재사용한다).
...
# 참조 파일 선택 전략 — lineart_batch.py의 사본 -----------------------------
...
# ============================================================================
# csv_batch.py / lineart_batch.py 공통 헬퍼 — 사본
# ============================================================================
...
def apply_prompts(workflow, row):
    ...
    for field, title_substring in PROMPT_FIELD_TITLES.items():
        ...
        if input_field is None:
            print(
                f"[lineart_csv_batch] 경고: '{field}' 값을 넣을 노드를 찾지 못했습니다 "
        ...
        applied = True
    if not applied:
        print("[lineart_csv_batch] 경고: 이 행에 프롬프트 컬럼 값이 하나도 없습니다.", file=sys.stderr)
...
def apply_seed(workflow, seed):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: 시드를 넣을 노드를 찾지 못했습니다 (KSampler 없음)", file=sys.stderr)
        return
    try:
        node.setdefault("inputs", {})["seed"] = int(seed)
    except (TypeError, ValueError):
        print(f"[lineart_csv_batch] 경고: seed 값 '{seed}'을 정수로 변환하지 못했습니다", file=sys.stderr)
...
def parse_batch_size(batch_no):
    ...
    try:
        ...
        print(f"[lineart_csv_batch] 경고: batch_no 값 '{batch_no}'을 정수로 변환하지 못했습니다", file=sys.stderr)
        return None
    return max(1, value)
...
def apply_batch_size(workflow, batch_no):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: batch_no를 넣을 노드를 찾지 못했습니다 (EmptyLatentImage 없음)", file=sys.stderr)
        return
...
def resolve_resolution(width, height, resolution):
    ...
    if width and height:
        try:
            ...
            print(
                f"[lineart_csv_batch] 경고: width/height 값 '{width}x{height}'을 정수로 변환하지 못했습니다. "
                "resolution 컬럼으로 대체합니다.",
                file=sys.stderr,
            )
    elif width or height:
        print(
            "[lineart_csv_batch] 경고: width/height는 둘 다 채워야 적용됩니다 (하나만 비어 있음). "
            "resolution 컬럼으로 대체합니다.",
            file=sys.stderr,
        )
    ...
    if not match:
        print(
            f"[lineart_csv_batch] 경고: resolution 값 '{resolution}'을 해석하지 못했습니다 "
...
def apply_checkpoint(workflow):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: 체크포인트를 넣을 노드를 찾지 못했습니다 (CheckpointLoader 없음)", file=sys.stderr)
        return
...
def apply_lora(workflow):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: LoRA를 넣을 노드를 찾지 못했습니다 (LoraLoader 없음)", file=sys.stderr)
        return
    total = sum(
    ...
    if total > 1:
        print(
            f"[lineart_csv_batch] 안내: LoRA 로더가 {total}개라 그중 노드 {node_id}에만 적용했습니다 "
    ...
    if strength:
        try:
            ...
            print(f"[lineart_csv_batch] 경고: LORA_STRENGTH 값 '{strength}'을 숫자로 변환하지 못했습니다.", file=sys.stderr)
        else:
...
def apply_resolution(workflow, width, height, resolution):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: 해상도를 넣을 노드를 찾지 못했습니다 (EmptyLatentImage 없음)", file=sys.stderr)
        return
...
def apply_filename_prefix(workflow, title, index, seed, char_no, lineart_path):
    node_id, node = find_node(
        workflow,
    ...
    if lineart_path is not None and char_no is not None and char_no != DEFAULT_CHAR_NO:
        prefix = f"{prefix}_char{char_no}"
    if lineart_path is not None:
        prefix = f"{prefix}_{sanitize_stem(lineart_path.stem)}"
...
def report_progress(job_id, nightshift_url, total, done):
    ...
    try:
        ...
        print(f"[lineart_csv_batch] 경고: 진행 상황 보고 실패: {e}", file=sys.stderr)


def append_manifest(output_dir, record):
    try:
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        path = Path(output_dir) / "lineart_csv_batch_manifest.jsonl"
        with open(path, "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")
    except Exception as e:
        print(f"[lineart_csv_batch] 경고: 재현성 기록 실패: {e}", file=sys.stderr)


# ============================================================================
# ComfyUI 연동 — lineart_batch.py의 사본
# ============================================================================
...
def apply_lineart_image(workflow, comfy_url, lineart_path):
    ...
    if node is None:
        print("[lineart_csv_batch] 경고: lineart 이미지를 넣을 노드를 찾지 못했습니다 (LoadImage 없음)", file=sys.stderr)
        return
...
def apply_secondary_reference(workflow, comfy_url, secondary_kind, ref_path):
    ...
    if node is None:
        print(
            f"[lineart_csv_batch] 안내: 보조 참조({secondary_kind}) 이미지를 넣을 노드(제목에 "
    ...
    if node.get("class_type") != "LoadImage":
        print(
            f"[lineart_csv_batch] 경고: SECONDARY_NODE_TITLE('{title}')로 찾은 노드가 LoadImage가 "
            "아니라 보조 참조를 건너뜁니다.",
            file=sys.stderr,
        )
        return
...
def disable_controlnet(workflow):
    """lineart 컬럼이 비어 있는 행 — 그래프를 재배선하지 않고 ControlNetApplyAdvanced류
    ...
    if node is None:
        print(
            "[lineart_csv_batch] 경고: ControlNet 노드를 찾지 못해 비활성화를 건너뜁니다 "
...
def build_plan(base_workflow, rows, lineart_dir, lineart_mode, secondary_kind=None, secondary_dir=None):
    """CSV 행마다 (row, title, seed, batch_size, char_no, lineart_path, secondary_path)를
    미리 계산한다. lineart/char_no(및 secondary_kind가 설정됐으면 secondary_ref/
    secondary_char_no) 컬럼 해석은 여기서 전부 끝내둔다 — 세트 지정 행은 이 단계에서
    피커를 소비해 실제 파일을 정하고, 해석에 실패하는 행이 하나라도 있으면 그
    자리에서 배치 전체를 중단한다(건너뛰지 않음) — lineart와 secondary_ref 둘 다 같은
    ...
    으로 캐시하면 서로 다른 세트가 피커 하나를 잘못 공유하게 된다. 주 참조(lineart)와
    ...
    def get_picker(cache, base_dir, char_no, set_name, label):
        ...
        if key not in cache:
            ...
            cache[key] = make_picker(lineart_mode, files)
        return cache[key]
    ...
    for line_no, row in enumerate(rows, start=2):  # 헤더가 1번 줄
        ...
        if not main_prompt:
            print(f"[lineart_csv_batch] 건너뜀 (main_prompt/prompt 없음, {line_no}번째 줄): {row}")
            continue
        ...
        lineart_raw = (row.get("lineart") or "").strip()
        char_no = None
        lineart_path = None
        if lineart_raw:
            try:
                ...
                print(
                    f"[lineart_csv_batch] 오류: {line_no}번째 줄의 char_no 값 "
                    f"'{row.get('char_no')}'을 정수로 변환하지 못했습니다.",
                    file=sys.stderr,
                )
                sys.exit(1)
            try:
                ref = resolve_ref_reference(lineart_dir, char_no, lineart_raw)
                if isinstance(ref, RefFileRef):
                    lineart_path = ref.path
                else:
                    lineart_path = get_picker(pickers, lineart_dir, char_no, ref.set_name, "lineart").pick()
            except RefReferenceError as e:
                print(
                    f"[lineart_csv_batch] 오류: {line_no}번째 줄의 lineart 값 '{lineart_raw}' "
        ...
        if secondary_kind:
            ...
            if secondary_raw:
                try:
                    ...
                    print(
                        f"[lineart_csv_batch] 오류: {line_no}번째 줄의 secondary_char_no 값 "
                ...
                try:
                    ...
                    print(
                        f"[lineart_csv_batch] 오류: {line_no}번째 줄의 secondary_ref 값 '{secondary_raw}' "
...
def run_once(base_workflow, comfy_url, row, title, seed, char_no, lineart_path, index, job_id, output_dir,
    ...
    apply_filename_prefix(workflow, title, index, seed, char_no, lineart_path)

    prompt_id = queue_prompt(comfy_url, workflow)
    lineart_label = f"{lineart_path.name}(char_no={char_no})" if lineart_path is not None else "(없음, ControlNet 비활성화)"
    secondary_log = f" secondary({secondary_kind})={secondary_path.name}" if secondary_path is not None else ""
    print(f"[lineart_csv_batch] [{index}] title={title!r} seed={seed} lineart={lineart_label}{secondary_log} 큐 등록 (prompt_id={prompt_id})")
    wait_for_completion(comfy_url, prompt_id)
    print(f"[lineart_csv_batch] [{index}] 완료")
    ...
    record = {
        ...
        "index": index,
        "char_no": char_no,
        "lineart_set": lineart_path.parent.name if lineart_path is not None else None,
        "lineart_file": lineart_path.name if lineart_path is not None else None,
        "seed": seed,
        "prompt_id": prompt_id,
    }
    if secondary_path is not None:
...
def main():
    ...
    if not workflow_path or not csv_path:
        print("[lineart_csv_batch] WORKFLOW_PATH와 CSV_PATH 환경변수가 모두 필요합니다.", file=sys.stderr)
        sys.exit(1)

    base_workflow = load_workflow(workflow_path)
    rows = load_rows(csv_path)
    if not rows:
        print("[lineart_csv_batch] CSV에 처리할 행이 없습니다.")
        return
    ...
    if secondary_kind not in REF_KINDS:
        if secondary_kind != "none":
            print(f"[lineart_csv_batch] 경고: 알 수 없는 SECONDARY_KIND '{secondary_kind}' — 보조 참조를 끕니다.", file=sys.stderr)
        secondary_kind = None
    else:
        secondary_dir = ref_kind_dir(secondary_kind)

    plan = build_plan(base_workflow, rows, lineart_dir, lineart_mode, secondary_kind, secondary_dir)
    if not plan:
        print("[lineart_csv_batch] 제출할 행이 없습니다 (모두 건너뜀).")
        return

    total_images = sum(item["batch_size"] for item in plan)
    print(f"[lineart_csv_batch] 총 {len(plan)}건 제출 예정, 이미지 {total_images}장 예상")
    if secondary_kind:
        print(f"[lineart_csv_batch] 보조 참조 사용 가능: 종류={secondary_kind}")
    ...
    for index, item in enumerate(plan, start=1):
        run_once(
            ...
            item["char_no"], item["lineart_path"], index, job_id, output_dir,
    ...
    print(f"[lineart_csv_batch] 총 {len(plan)}건 완료 (이미지 {done_images}장)")
